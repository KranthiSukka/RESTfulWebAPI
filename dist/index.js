...
//Todo App:
var cosmosClient = new CosmosClient.CosmosClient({
    endpoint: config.host,
    auth: {
        masterKey: config.authKey
    }
});
var taskDao = new TaskDao.TaskDao(cosmosClient, config.databaseId, config.containerId);
var taskList = new TaskList.TaskList(taskDao);
taskDao.init()
    .catch(function (err) {
    console.error(err);
...
server.route({
    ...
    path: '/updatetask',
    handler: function (request, h) {
        taskList.completeTask(request, h);
        //return 'Hello, ' + encodeURIComponent(request.params.name) + '!';
    }
});
server.route({
